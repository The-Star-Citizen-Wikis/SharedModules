...
function Starmap.link(location, system)
    ...
    if location and system then
        str = str .. '&' .. 'system=' .. system
    end
...
function Starmap.findStructure(structureType, structureName)
    ...
    for _, structure in ipairs(structures) do
        if string.lower(structure['name'] or '') == structureName or string.lower(structure['designation'] or '') ==
            structureName or string.lower(structure['code'] or '') == structureName then
            return structure
        end
    end
...
function Starmap.pathTo(target)
    ...
    local function processStructure(structure)
        ...
        if parentType == 'system' then
            table.insert(links, '[[' .. removeParentheses(parent['name']) .. ' system]]')
        elseif parent.type == 'STAR' then
            local designation = removeParentheses(parent['designation'])
            table.insert(links, '[[' .. designation .. ' (star)|' .. designation .. ' star]]')
        else
...
function Starmap.main(frame)
    ...
    if structure then
        ...
        local system = nil
        if structure['star_system'] then
            system = structure['star_system']['code']
        end
