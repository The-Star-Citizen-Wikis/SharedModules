...
local function formatRequiredByList( moduleName, addCategories, whatLinksHere )
    local childModuleData = arr.map( whatLinksHere or {}, function ( title )
...
function p._main( currentPageName, addCategories, isUsed )
    ...
    local whatTemplatesLinkHere = arr{}
    local whatModulesLinkHere = arr{}
    ...
    for _, link in ipairs( templatesRes ) do
        local name = cleanFrom( link[ 'from' ] )
        if name ~= nil then
            table.insert( whatTemplatesLinkHere, name )
        end
    end
    ...
    for _, link in ipairs( moduleRes ) do
        local name = cleanFrom( link[ 'from' ] )
        if name ~= nil and name ~= currentPageName then
            table.insert( whatModulesLinkHere, name )
        end
    end

    whatModulesLinkHere = whatModulesLinkHere:unique()
    whatTemplatesLinkHere = whatTemplatesLinkHere:unique()
