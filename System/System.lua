...
function System.main(frame)
    ...
    if (name == nil or name == '') and system['name'] then
        name = system['name']
    end
    if (code == nil or code == '') and system['code'] then
        code = system['code']
    end
    ...
    if type then
        translatedType = t('val_type_' .. string.lower(type))
    end
    if status then
        translatedStatus = t('val_status_' .. string.lower(status))
    end
    ...
    local sensorDanger = inlineIf(args['danger'], tonumber(args['danger']),
        tonumber(e(system, 'aggregated_danger') or 0))
    local sensorEconomy = inlineIf(args['economy'], tonumber(args['economy']),
        tonumber(e(system, 'aggregated_economy') or 0))
    local sensorPopulation = inlineIf(args['population'], tonumber(args['population']),
        tonumber(e(system, 'aggregated_population') or 0))
    ...
    local size = inlineIf(system['aggregated_size'], tonumber(system['aggregated_size']), nil)
    local affiliationLink = nil
    if affiliation then
        affiliationLink = inlineIf(startsWith(affiliation, '[['), '', '[[') .. affiliation ..
                              inlineIf(endsWith(affiliation, ']]'), '', ']]')
    end
    ...
    infobox:renderSection({
        content = {infobox:renderItem({
            ...
            label = t('lbl_size'),
            data = inlineIf(size, tostring(size) .. ' AU', args['size'])
        }), infobox:renderItem({
            label = t('lbl_status'),
    ...
    frame:callParserFunction('SHORTDESC',
        string.format(inlineIf(tonumber(planets) == 1, t('shortdesc_singular'), t('shortdesc_plural')),
            string.gsub(string.lower(translatedType), '^%l', string.upper), planets))

    local categories = {'Systems'}
    if affiliation then
        table.insert(categories, affiliation .. ' Systems')
    end
